...
dim upload_type,product,picsize,pmylogo
...
next
	...
	FileNamenew=replace(FileName,"uploadimages/","")
	if pmylogo="1" then
		FileNamenew = "logo.jpg"
	end if
	set fso=server.CreateObject("Scripting.FileSystemObject")
	set f2=fso.getfile(server.MapPath(""&FileName&""))
	f2.copy(server.MapPath(""&FileNamenew&""))
	
	response.write "图片路径是:images/"&FileNamenew 
	response.write "<script language=JavaScript>" & "window.opener.document.all."&picsize&".value ='images/"&FileNamenew&"';" &"alert('图片上传成功!');"& "window.close();" & "</script>"
	response.end
end sub
...
function MakedownName(filename)
  ...
  for u=0 to ubound(Forumupload)
		if instr(filename,Forumupload(u))>0 then
			MakedownName=filename   ' fname & "." & Forumupload(u)
			exit for
		end if
  next
end function

  ...
  <tr>
    <td> 
		...
		<TABLE width="960" border=0 align="center" cellPadding=0 cellSpacing=2 class=a1>
		  <TBODY>
			<TR> 
			  <TD width=540 height=50 background=images/buystep0.jpg>
	...
	<TABLE width="960" align="center" border=0 cellPadding=0 cellSpacing=0 background="images/bg4.gif" style="margin-top:8px;">
	  <TBODY>
		<TR>
				  <TD width=128 height=30 background=images/hotnewpro.gif>&nbsp;&nbsp;&nbsp;&nbsp;<span class="style11">
				  <%
					if cx="1" then
						response.write "促销商品列表"
					elseif hot="1" then
						response.write "热卖商品列表"
					else
						response.write "商品列表"
					end if
					%></span></TD>
...
if rs.bof or rs.eof then
	response.write "暂时没有符合条件商品记录！<br><br><br><br>"
else
...
<%elseif display_mode="list" then %>
			...
			<%if s_model="是" then%>
			商品号：<font style="width:98px;"><%=model%></font>
			<%end if%>
			<%if s_pipai="是" then%>
			品　牌：<%=pipai%>
			<%end if%>
			...
			<DIV align="left" style="MARGIN-TOP: 2px;">
			<%=session("customkindname")%>价：<font class='nowprice' style="width:80px;">￥<%=price2%></font>市场价：<FONT class='oldprice' style="width:70px;"><del>￥<%=price1%></del></FONT>
			上市时间：<%=addtime%>
			</DIV>
			</td>
			<td>
...
response.write "<table width='650' align='center'>"
  response.write "<tr> "
   response.write " <td width='500' height='22' align=right style='font-size:9pt;'> "
		response.write "共"&rs.recordcount&"条记录&nbsp;每页"&s_col*s_row&"条&nbsp;现在 "&nowpage&"/"&rs.pagecount &" 页&nbsp;"	
    ...
    response.write "</td><form name='form29' method='post' action='"&nowpath&"?"&myurl&"&none='><td width='150'>&nbsp;转到<input type='text' name='page' size=3 style='width:30px; height:18px;font-size:12pt;font-weight:bold;font-family: Arial;'>页 <input type='submit' name='Submitpage' value='GO' style='width:30px; height:22px; color:#ffffff; background-color:#999999;font-size:10pt;font-weight:bold;font-family: Arial; '></td></form>"
  response.write "</tr>"

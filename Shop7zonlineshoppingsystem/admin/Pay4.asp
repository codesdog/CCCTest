...
if ok="ok" then
		...
		sql="update sh_pay4 set youstat='"&youstat&"',memo='"&memo&"' "
		conn.execute(sql)

	response.write "<script language=JavaScript>" & "alert('成功保存设置！');" & "window.location.href='pay4.asp';" & "</script>"
	conn.close
	response.end
end if
%>
...
>
  <TBODY>
    <TR>
      <TD width="90%" bgColor=#f4f5fd><font color="#000000">　<B><font color="#FF0000">邮局汇款设置</font></B></font></TD>
...
<table width="93%"  border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#b1bfee">
  <form action=pay4.asp method=post name=setup>
    <tr bgcolor="#FFFFFF"> 
    ...
    <tr bgcolor="#FFFFFF"> 
      <td height="30" align="center" valign="middle"><div align="right">邮局汇款说明内容：</div></td>
